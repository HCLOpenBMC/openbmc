...
SRC_URI_append = " file://hand_switch_position_update.sh"
SRC_URI_append = " file://hand_switch_position_update.service"

SYSTEMD_PACKAGES = "${PN}"
SYSTEMD_SERVICE_${PN} = "hand_switch_position.service phosphor-multi-gpio-monitor.service hand_switch_position_update.service"
...
do_install_append() {
 ...
 install -m 0644 -D ${WORKDIR}/hand_switch_position_update.service \
                   ${D}/lib/systemd/system  

     install -m 0755 -D ${WORKDIR}/hand_switch_position.sh \
                   ${D}/usr/bin

     install -m 0755 -D ${WORKDIR}/hand_switch_position_update.sh \
                   ${D}/usr/bin
